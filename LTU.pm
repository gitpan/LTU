...
##
##  CONSTANTS USABLE FROM THE OUTSIDE
##
...
sub restore {
    ...
    if ($ltu_version ne $LTU::VERSION) {
	carp "restore: $file_name written with LTU version $ltu_version.\n";
	carp "This is version $LTU::VERSION.  LTU not restored.\n";
	return(0);
    }
...
##  Returns 1 iff $instance is on the same side of LTU::THRESHOLD
##  as $value is.
##
sub correctly_classifies {
    ...
    ((($actual_value   < $LTU::THRESHOLD) and
      ($desired_value  < $LTU::THRESHOLD)) or
     (($actual_value  >= $LTU::THRESHOLD) and
      ($desired_value >= $LTU::THRESHOLD)));
}
...
sub eval_on_set {
	...
	if ($ActualValue >= $LTU::THRESHOLD) {
	    if ($DesiredValue >= $LTU::THRESHOLD) {
		$TP++;
	    } else {
		$FP++;
	    }
	} elsif ($DesiredValue >= $LTU::THRESHOLD) {
	    $FN++;
	} else {
	    $TN++;
	}
...
sub train { 
    ...
    return(1) if ((($actual_value   < $LTU::THRESHOLD) && 
		   ($desired_value  < $LTU::THRESHOLD)) ||
		  (($actual_value  >= $LTU::THRESHOLD) && 
		   ($desired_value >= $LTU::THRESHOLD)));
    ...
    $delta = -$delta   if $desired_value < $LTU::THRESHOLD;
...
###
package RLS;
@RLS::Inherit::ISA = @RLS::ISA = qw( LTU );

sub new {
    my($type, $nvars, $scaling) = @_;

    my($self) = $type->RLS::Inherit::new($nvars, $scaling);
...
sub copy {
    ...
    my($new) = $self->RLS::Inherit::copy($self);
...
##
##  RLS::TRAIN trains the specified linear threshold unit on a particular
...
##
##  LMS::TRAIN trains the specified linear threshold unit on a particular
##  instance_vector.  It returns 1 if the linear threshold unit already
##  classified the instance_vector correctly, otherwise it returns 0.
...
##
##  TACR::TRAIN trains the specified linear threshold unit on a particular
##  instance_vector.  It returns 1 if the linear threshold unit already
##  classified the instance_vector correctly, otherwise it returns 0.
